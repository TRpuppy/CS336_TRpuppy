...
class scaled_dot_product_attention(nn.Module):
    ...
    mask: Float[Tensor, " ... queries keys"] | None = None) -> Float[Tensor, " ... queries d_v"]:
        if mask is None:
            # Expand mask to match Q's batch dimensions: (..., queries, keys)
            mask = torch.ones(*Q.shape[:-2], Q.shape[-2], K.shape[-2], device=self.device, dtype=self.dtype)
        d_k = Q.shape[-1]
        QK_norm = einsum(Q, K, " ... queries d_k, ... keys d_k -> ... queries keys") / (d_k ** 0.5)
        QK_norm[mask == 0] = -torch.inf
...
class TransformerBlock(nn.Module):
    def __init__(self, d_model: int, num_heads: int, d_ff: int, theta: float, max_seq_len: int, device=None, dtype=None):
        ...
        self.num_heads = num_heads
        self.d_ff = d_ff
        self.msa_RoPE = RoPE(theta, d_model//num_heads, max_seq_len)
        self.msa = multihead_self_attention(d_in=d_model, d_out=d_model, num_heads=num_heads, d_k=d_ff, d_v=d_ff, RoPE=self.msa_RoPE, device=device, dtype=dtype)
